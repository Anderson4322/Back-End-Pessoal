...
app.post("/cadastro/user", async (req, res) => {
  const { nome, email, senha, endereco, tipo_de_conta} = req.body;
  await sql`insert into usuarios(tipoConta,nome, email, senha, endereco) values (${tipo_de_conta},${nome}, ${email}, ${senha}, ${endereco})`;
...
app.put("/alt_user/:id", async (req, res) => {
  ...
  await sql`UPDATE usuarios SET nome=${nome}, email=${email}, tipoconta=${tipoconta}, senha=${senha} WHERE id_usuario = ${id};`;
  return res.status(201).json("alterado");
});
